...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'tblend ?(.*)', fromMe: true, desc: 'T_DESC'}, (async (message, match) => {    

        if (message.reply_message === false) return await message.client.sendMessage(message.jid,'MP4TOAUDİO_NEEDREPLY', MessageType.text);

        if (message.repy_message && match[1] === '') return await message.client.sendMessage(message.jid, 'T_NOT', MessageType.text);
        ...
        if (match[1] === 'dodge') {
            ...
            return await message.client.sendMessage(message.jid, 'T_FALSE', MessageType.text);
        }
    }));
}
else if (Config.WORKTYPE == 'public') {

    Asena.addCommand({pattern: 'tblend ?(.*)', fromMe: false, desc: 'T_DESC'}, (async (message, match) => {    
